...
const storeUser = (req, res, next) => {
  ...
  userModel
    .create({ name, email, imagepath, password })
    .then((data) => {
      var token = jwt.sign({ user_id: data._id }, "mohamed2562289mbn", {
        expiresIn: "2h",
      });
      data["token"] = token;
      console.log(data);
...
const updateUser = (req, res) => {
  ...
  var email = req.body.email;
  userModel
    .findByIdAndUpdate({ _id: id }, { name: name, email: email })
    .then((data) => {
      res.status(201).json({ staus: true, data: data });
    })
    .catch((err) => {
...
const updatePassword = async (req, res) => {
  ...
  userModel
    .findByIdAndUpdate({ _id: id }, { password: passwordUpdate })
    .then((data) => {
      res.status(201).json({ staus: true, data: data });
    })
    .catch((err) => {
...
const login = (req, res) => {
  ...
  userModel
    ...
    .then(async (data) => {
      ...
      if (password) {
        const token = jwt.sign({ user_id: data._id }, "mohamed2562289mbn", {
          expiresIn: "2h",
        });
        data.token = token;
        res.status(201).json({ staus: true, data: data });
